...
namespace Content.Server.Inkymed.Systems;
...
public sealed partial class DefibrillatorHeartSystem : EntitySystem // slop
{
    ...
    private void OnAfterInteract(Entity<ManualDefibrillatorComponent> ent, ref AfterInteractEvent args)
    {
        ...
        EntityUid heartUid = default;
        HeartComponent? heart = null;
        var query = EntityQueryEnumerator<HeartComponent>();
        while (query.MoveNext(out var hUid, out var hComp))
        {
            if (Transform(hUid).ParentUid == target)
            {
                heartUid = hUid;
                heart = hComp;
                break;
    ...
    private void OnFibbed(EntityUid target, BodyComponent body, ref TargetDefibrillatedEvent args)
    {
        ...
        EntityUid heartUid = default;
        HeartComponent? heart = null;
        var query2 = EntityQueryEnumerator<HeartComponent>();
        while (query2.MoveNext(out var hUid, out var hComp))
        {
            if (Transform(hUid).ParentUid == target)
            {
                heartUid = hUid;
